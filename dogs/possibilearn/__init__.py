...
def possibility_learn(x, mu, c=1, k=GaussianKernel(), sample_generator=None,
                      adjustment=0, fuzzifier=LinearFuzzifier()):
...
'''
    ...
    try:
        chis = solve_optimization(x, mu, c=c, k=k, adjustment=adjustment)
    except ValueError:
        return (None, np.inf)
    ...
    chi_SV_index = [i for i in range(len(chis)) if -c*(1-mu[i]) < chis[i] < c*mu[i]]
    ...
    # max_radius = np.max(map(estimated_square_distance_from_center, sample))
    #
    ...
    #              else result if result > 0 else 0
...
def split_indices(xs, mus, percentages, shuffle=True):
    '''Generate a holdout triple of training-validate-test set.

    Arguments:

    - xs: list of objects
    - mus: list of paired membership values
    ...
    values_train = [xs[i] for i in indices[:num_train]]
    values_validate = [xs[i] for i in indices[num_train:num_train+num_validate]]
    values_test = [xs[i] for i in indices[num_train+num_validate:]]
...
'''
    ...
    for c, k in iterator:
        ...
        fold_num = 0
        for (x_train, x_test, mu_train, mu_test) in folded_data:
            if verbose:
...
def model_selection_holdout(x_train, mu_train,
                            x_validate, mu_validate,
...
- x_train: iterable of pairs of objects to be used for training
- mu_train: iterable of pairs membership degrees for objects to be used
  for training
- x_validate: iterable of paired objects to be used for model selection
- mu_validate: iterable of paired membership degrees for objects to be
