...
class QuantileConstantPiecewiseFuzzifier(BaseFuzzifier):
    ...
    def get_fuzzified_membership(self, SV_square_distance, sample,
        ...
        m = np.median([s for s in sample if s > SV_square_distance])
        q1 = np.percentile([s for s in sample if s > SV_square_distance], 25)
        q3 = np.percentile([s for s in sample if s > SV_square_distance], 75)
    ...
    def get_fuzzified_membership(self, SV_square_distance, sample,
        ...
        def estimated_membership(x):
            ...
            return 1 if r <= ssd \
                 ...
                 else (-r + ssd + q3)/(4*(mx-q3)) + 1./4 if r <= ssd+mx else 0
...
class ExponentialFuzzifier(BaseFuzzifier):
    def __init__(self, k):
        self.k = k
        self.name = 'Exponential({})'.format(k)
    ...
    def get_fuzzified_membership(self, SV_square_distance, sample,
        ...
        def estimated_membership(x):
            r = estimated_square_distance_from_center(np.array(x))
            return 1 if r <= ssd \
                     else np.exp(-np.log(self.k)/m * (r - SV_square_distance))
        return estimated_membership
